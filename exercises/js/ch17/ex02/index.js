...
// GitHubのIssueを作成する関数
export function createIssue(headers, user, repo, title, body) {
  // リクエストのボディに書き込むJSON
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        logResponseData(responseBody, verbose);
        resolve(JSON.parse(responseBody));
        console.log(`ID: ${JSON.parse(responseBody).id}`);
      });
    });
...
export function closeIssue(headers, user, repo, issueNumber) {
  // リクエストのボディに書き込むJSON
  const data = JSON.stringify({
    state: 'closed'
  });
...
// オープンなIssue一覧を取得する関数
export function listOpenIssues(headers, user, repo) {
  const options = {
    hostname: 'api.github.com',
      ...
      res.on('end', () => {
        ...
        try {
          ...
          issues.forEach(issue => {
            console.log(`ID: ${issue.id}, Title: ${issue.title}, URL: ${issue.url}`);
          });
          resolve(issues);
        } catch (e) {
          reject(e);
...
if (args.includes('create')) {
  ...
  createIssue(headers, user, repo, title, body);
} else if (args.includes('close')) {
  const issueNumber = args[args.indexOf('close') + 1];
  closeIssue(headers, user, repo, issueNumber);
} else if (args.includes('list')) {
  listOpenIssues(headers, user, repo);
}
